        ...
        <ul className="list-group list-movie m-auto">
          {
            this.state.movies.map((value, key) => <li key={key} className="list-group-item mb-2 bg-dark">
              <a href={value.url}>
                {value.title}<br />
                <div className="">
                  {console.log(value.tomatoIcon)}
                  <Icon type={value.tomatoIcon} size="tiny" /> {value.tomatoScore}%
                  <Icon type={value.popcornIcon} size="tiny" /> {value.popcornScore}% {value.theaterReleaseDate}
